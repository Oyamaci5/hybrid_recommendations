"""
Run COA-KMeans-MF experiment on MovieLens 100K.
Set RUN_COA_KMEANS_EXPERIMENT = True to run when executing this script directly.
"""

# Set to True to run when this script is executed directly; False to skip
RUN_COA_KMEANS_EXPERIMENT = False
...
def save_results(results: dict, config: Config, method_name: str, **kwargs):
    ...
    summary = {
        ...
        'n_coatis': int(kwargs.get('n_coatis', 30)),
        ...
        'coa_iterations': int(kwargs.get('coa_iterations', 50)),
        'kmeans_iterations': int(kwargs.get('kmeans_iterations', 100)),
        'n_iterations': int(kwargs.get('coa_iterations', 50) + kwargs.get('kmeans_iterations', 100)),
...
def run_coa_kmeans_mf_experiment(config: Config = None, n_coatis: int = 30,
    ...
    Args:
        ...
        coa_iterations: Number of COA iterations
    ...
    if verbose:
        print("=" * 60)
        print("COA-KMeans-MF Experiment")
        print("=" * 60)
        print(f"Dataset: {config.data_dir}")
        print(f"Train: {n_train} ratings, Test: {n_test} ratings")
        print(f"Users: {n_users}, Items: {n_items}")
        print(f"Latent dim: {config.latent_dim}")
        print(f"COA: {n_coatis} coatis, {coa_iterations} iterations")
    ...
    save_results(results, config, 'coa_kmeans_mf', n_coatis=n_coatis,
...
if __name__ == "__main__":
    if not RUN_COA_KMEANS_EXPERIMENT:
        print("COA-KMeans-MF experiment skipped (RUN_COA_KMEANS_EXPERIMENT=False). Set to True to run.")
        exit(0)
    config = Config()
    run_coa_kmeans_mf_experiment(config, verbose=True)

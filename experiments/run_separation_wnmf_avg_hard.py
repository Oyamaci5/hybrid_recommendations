"""
Separation-protocol: CalcAvgRating (--cluster-avg-hard), sharedV/global/kNN yok.
"""
...
def main() -> None:
    ...
    for protocol, suffix_tmpl, ks in JOBS:
        for k in ks:
            ...
            print("\n" + "=" * 72)
            print(f"{protocol}  K={k}  (calc_avg_rating)")
            print(" ".join(cmd))
            print("=" * 72)
            if subprocess.run(cmd, cwd=str(REPO)).returncode != 0:
                sys.exit(1)
    print("\nBitti. CSV: results/wnmf/ml100k/k<K>/fold1/run*/ (Senaryo=calc_avg_rating)")
